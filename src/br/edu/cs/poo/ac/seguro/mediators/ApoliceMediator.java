...
public class ApoliceMediator {
	...
	public RetornoInclusaoApolice incluirApolice(DadosVeiculo dados) {
	    ...
	    for (Sinistro sin : sinistros) {
            ...
            if (vSin == null || vSin.getProprietario() == null) continue;
            
            boolean mesmoProprietario = false;
            if(segurado.isEmpresa() == vSin.getProprietario().isEmpresa()){
                if(segurado.isEmpresa()){
                    mesmoProprietario = ((SeguradoEmpresa)segurado).getCnpj().equals(((SeguradoEmpresa)vSin.getProprietario()).getCnpj());
                } else {
                    mesmoProprietario = ((SeguradoPessoa)segurado).getCpf().equals(((SeguradoPessoa)vSin.getProprietario()).getCpf());
                }
            }

	        if (vSin.getPlaca().equalsIgnoreCase(veiculo.getPlaca()) &&
